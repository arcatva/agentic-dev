...
use axum::{routing::{get, post, put, patch, delete}, Router, middleware::{self, Next}, extract::{State, Request, DefaultBodyLimit}, response::{IntoResponse, Response}, http::StatusCode, Json};
use serde_json::json;
...
pub fn app(state: AppState) -> Router {
    ...
    let compressed = Router::new()
        ...
        .route("/api/sessions/{id}/commits/{sha}/diff", get(sessions::commit_diff_route))
        ...
        .route("/api/uploads", post(sessions::upload_staging_route)
            .layer(DefaultBodyLimit::max(upload_limit)))
        ...
        .route("/api/skills/sources", get(misc::skills_sources_route).post(misc::skills_sources_add_route).delete(misc::skills_sources_delete_route))
...
async fn tls_cert_pem(State(st): State<AppState>) -> Response {
    ...
    match tokio::fs::read(&path).await {
        ...
        Ok(bytes) => {
            ...
            ([(axum::http::header::CONTENT_TYPE, "application/x-pem-file")], pem).into_response()
        }
...
mod tests {
    ...
    async fn catch_panic_layer_converts_panic_to_logged_500_json() {
        ...
        async fn boom() -> &'static str { panic!("kaboom") }
        let app = Router::new()
            .route("/boom", get(boom))
            .layer(CatchPanicLayer::custom(handle_panic));
        let resp = app
    ...
    async fn login_rejects_bad_password() {
        let resp = app(test_state().await).oneshot(with_ip(
    ...
    async fn login_with_empty_body_is_401_not_415() {
        ...
        let resp = app(test_state().await).oneshot(with_ip(
            Request::post("/api/login").body(Body::empty()).unwrap())).await.unwrap();
        assert_eq!(resp.status(), StatusCode::UNAUTHORIZED);
    }
    ...
    async fn login_rejects_shorter_and_longer_passwords() {
        ...
        let r1 = app(test_state().await).oneshot(with_ip(
            Request::post("/api/login").header("content-type", "application/json")
                .body(Body::from(r#"{"password":"p"}"#)).unwrap())).await.unwrap();
        assert_eq!(r1.status(), StatusCode::UNAUTHORIZED, "shorter password must 401");
        // Much longer than "pw"
        let r2 = app(test_state().await).oneshot(with_ip(
            Request::post("/api/login").header("content-type", "application/json")
                .body(Body::from(r#"{"password":"password_much_longer_than_two_chars"}"#)).unwrap())).await.unwrap();
        assert_eq!(r2.status(), StatusCode::UNAUTHORIZED, "longer password must 401");
        // Correct password still 200
        let r3 = app(test_state().await).oneshot(with_ip(
            Request::post("/api/login").header("content-type", "application/json")
                .body(Body::from(r#"{"password":"pw"}"#)).unwrap())).await.unwrap();
    ...
    async fn login_without_connect_info_does_not_500() {
        ...
        let r1 = app(test_state().await).oneshot(
            Request::post("/api/login").header("content-type", "application/json")
                .body(Body::from(r#"{"password":"wrong"}"#)).unwrap()).await.unwrap();
        assert_eq!(r1.status(), StatusCode::UNAUTHORIZED, "bad pw without ConnectInfo must 401");
        // Correct password — no ConnectInfo extension, must 200.
        let r2 = app(test_state().await).oneshot(
            Request::post("/api/login").header("content-type", "application/json")
                .body(Body::from(r#"{"password":"pw"}"#)).unwrap()).await.unwrap();
        assert_eq!(r2.status(), StatusCode::OK, "correct pw without ConnectInfo must 200");
    }
    ...
    async fn api_gate_blocks_without_token_and_allows_with() {
        // no token
        let r1 = app(test_state().await).oneshot(
            Request::get("/api/sessions").body(Body::empty()).unwrap()).await.unwrap();
        assert_eq!(r1.status(), StatusCode::UNAUTHORIZED);
        // with a valid bearer token
        let token = issue_token("s3cret", 3600, now_secs());
        let r2 = app(test_state().await).oneshot(
            Request::get("/api/sessions").header("authorization", format!("Bearer {token}"))
                .body(Body::empty()).unwrap()).await.unwrap();
        assert_eq!(r2.status(), StatusCode::OK);
        // with ?token= (WS-style)
        let r3 = app(test_state().await).oneshot(
            Request::get(format!("/api/sessions?token={token}")).body(Body::empty()).unwrap()).await.unwrap();
    ...
    async fn api_gate_rejects_every_bad_credential_shape() {
        ...
        let cases: Vec<(&str, Option<String>)> = vec![
            ("missing header", None),
            ("no Bearer prefix", Some(issue_token("s3cret", 3600, now_secs()))), // raw token, no "Bearer "
            ("wrong scheme", Some(format!("Basic {}", issue_token("s3cret", 3600, now_secs())))),
        ...
        for (label, header) in cases {
            ...
            if let Some(h) = header { b = b.header("authorization", h); }
            let resp = app(test_state().await).oneshot(b.body(Body::empty()).unwrap()).await.unwrap();
            assert_eq!(resp.status(), StatusCode::UNAUTHORIZED, "case `{label}` must 401");
            let v = body_json(resp).await;
        ...
        let ok = app(test_state().await).oneshot(
            Request::get("/api/sessions").header("authorization", format!("Bearer {good}"))
                .body(Body::empty()).unwrap()).await.unwrap();
        assert_eq!(ok.status(), StatusCode::OK, "valid token must pass the gate");
    }
    ...
    async fn healthz_and_login_bypass_the_auth_gate() {
        ...
        let h = app(test_state().await).oneshot(
        ...
        let l = app(test_state().await).oneshot(with_ip(
            Request::post("/api/login").header("content-type", "application/json")
                .body(Body::from(r#"{"password":"pw"}"#)).unwrap())).await.unwrap();
        assert_eq!(l.status(), StatusCode::OK, "/api/login must be reachable without a token");
    ...
    async fn login_throttle_locks_same_ip_after_eight_bad_attempts() {
        ...
        for i in 0..8 {
            let resp = app(st.clone()).oneshot(with_ip(
                Request::post("/api/login").header("content-type", "application/json")
                    .body(Body::from(r#"{"password":"wrong"}"#)).unwrap())).await.unwrap();
            assert_eq!(resp.status(), StatusCode::UNAUTHORIZED, "bad attempt {i} should 401");
            let v = body_json(resp).await;
            assert_eq!(v["error"], "bad password", "attempt {i} body");
        }
        // 9th attempt from the same IP is now locked out.
        let locked = app(st.clone()).oneshot(with_ip(
            Request::post("/api/login").header("content-type", "application/json")
                .body(Body::from(r#"{"password":"wrong"}"#)).unwrap())).await.unwrap();
        assert_eq!(locked.status(), StatusCode::TOO_MANY_REQUESTS, "9th attempt must be throttled");
        ...
        let still_locked = app(st.clone()).oneshot(with_ip(
            Request::post("/api/login").header("content-type", "application/json")
                .body(Body::from(r#"{"password":"pw"}"#)).unwrap())).await.unwrap();
    ...
    async fn unknown_api_route_with_valid_token_is_404() {
        let token = issue_token("s3cret", 3600, now_secs());
        let resp = app(test_state().await).oneshot(
    ...
    async fn malformed_json_body_on_post_is_client_error_not_500() {
        let resp = app(test_state().await).oneshot(with_ip(
            Request::post("/api/login").header("content-type", "application/json")
                .body(Body::from("{not valid json")).unwrap())).await.unwrap();
        let s = resp.status();
