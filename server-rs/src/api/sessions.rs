...
pub async fn get_session(
    ...
    match out {
        Ok((lines, start, total)) => {
            Json(json!({ "session": session, "log": lines, "start": start, "total": total }))
                .into_response()
        }
        Err(_) => {
...
pub struct CreateBody {
    ...
    #[serde(rename = "hiddenPlugins")]
    pub hidden_plugins: Option<Vec<String>>,
...
fn validate_disjoint(hidden: &[String], forced_on: &[String], kind: &str) -> Option<String> {
    ...
    for id in forced_on {
        ...
        if hidden_set.contains(id) {
            return Some(format!(
                "{kind}: \"{id}\" appears in both hidden and forcedOn lists — choose one"
            ));
        }
    }
    None
...
pub async fn create_session(State(st): State<AppState>, body: Bytes) -> Response {
    ...
    for def in &extra_mcp_servers {
        if def.name.trim().is_empty() {
            return (
                StatusCode::BAD_REQUEST,
                Json(json!({"error":"extraMcpServers: name must be non-empty"})),
            )
                .into_response();
        }
    ...
    if let Some(msg) = validate_disjoint(
        &hidden_plugins,
        &forced_on_plugins,
        "hiddenPlugins/forcedOnPlugins",
    ) {
    ...
    if let Some(msg) = validate_disjoint(
        &hidden_mcp,
        &forced_on_mcp,
        "hiddenMcpServers/forcedOnMcpServers",
    ) {
        return (StatusCode::BAD_REQUEST, Json(json!({"error": msg}))).into_response();
    }
    ...
    match st
        .engine
        .submit_session(repos, skills, prompt, HashMap::new(), meta)
        .await
    {
...
pub async fn patch_session(
    ...
    let patch = SessionPatch {
        ...
        group_id: b
            .group_id
            .map(|gid| if gid.is_empty() { None } else { Some(gid) }),
        ...
        auto_resume_at: if b.auto_resume == Some(false) {
            Some(None)
        } else {
            None
        },
...
pub async fn post_message(
        ...
        .await
    {
        Ok(raw_since) => {
            ...
            let since = crate::engine::transcript::raw_to_rendered_offset(
                &st.engine.get_log(&id),
                raw_since as usize,
            );
...
pub async fn permission_route(
    ...
    if decision != "allow" && decision != "deny" {
        return (
            StatusCode::BAD_REQUEST,
            Json(json!({"error":"decision must be 'allow' or 'deny'"})),
        )
            .into_response();
    }
    st.engine
        .respond_permission(&id, &decision, b.feedback.as_deref());
    Json(json!({"ok": true})).into_response()
}
...
pub async fn fork_session_route(State(st): State<AppState>, Path(id): Path<String>) -> Response {
    match st.engine.fork_session(&id).await {
        Ok(session) => (
            StatusCode::CREATED,
            Json(json!({ "id": session.id, "session": session })),
        )
            .into_response(),
        Err(e) => engine_error_response(e),
    }
...
pub async fn workflows_route(State(st): State<AppState>, Path(id): Path<String>) -> Response {
    ...
    let runs = match s.claude_session_id.as_deref() {
        Some(uuid) => {
            crate::engine::workflows::list_workflows(&st.config.claude_config_base, Some(uuid))
        }
        None => Vec::new(),
    };
...
pub async fn rewind_route(
    ...
    let Some(turn_index) = b.turn_index else {
        return (
            StatusCode::BAD_REQUEST,
            Json(json!({"error":"turnIndex required"})),
        )
            .into_response();
    };
...
fn parse_range(header: Option<&str>, if_range: Option<&str>, etag: &str, len: u64) -> RangeOutcome {
    ...
    if spec.contains(',') {
        return RangeOutcome::Full;
    } // multi-range: serve full instead
    ...
    match (a, b) {
        ...
        ("", suf) => match suf.parse::<u64>() {
            ...
            Ok(n) if len > 0 => RangeOutcome::Slice {
        ...
        (s, "") => match s.parse::<u64>() {
            Ok(start) if start < len => RangeOutcome::Slice {
                start,
                end: len - 1,
            },
        ...
        (s, e) => match (s.parse::<u64>(), e.parse::<u64>()) {
            (Ok(start), Ok(end)) if start <= end && start < len => RangeOutcome::Slice {
                start,
                end: end.min(len - 1),
            },
...
pub async fn upload_route(
    ...
    if s.worktree_path.as_deref().unwrap_or("").is_empty() {
        return (
            StatusCode::BAD_REQUEST,
            Json(json!({"error":"session has no worktree"})),
        )
            .into_response();
    }
    ...
    if std::fs::create_dir_all(&dir).is_err() {
        return (
            StatusCode::BAD_REQUEST,
            Json(json!({"error":"cannot create uploads dir"})),
        )
            .into_response();
    }
...
pub async fn upload_staging_route(
    State(st): State<AppState>,
    mut multipart: axum::extract::Multipart,
) -> Response {
    let field = match multipart.next_field().await {
    ...
    let staged = tokio::task::spawn_blocking(move || -> std::io::Result<(String, String)> {
        crate::engine::sweep_stale_staging(
            &worktrees_root,
...
fn collect_outbox(base: &std::path::Path, label: &str, out: &mut Vec<serde_json::Value>) {
    ...
    let mut stack: Vec<(std::path::PathBuf, String, usize)> =
        vec![(base.to_path_buf(), label.to_string(), 0)];
...
pub async fn outbox_route(State(st): State<AppState>, Path(id): Path<String>) -> Response {
    ...
    let mut dirs: Vec<(std::path::PathBuf, String)> =
        vec![(session_cwd(&s).join("outbox"), "outbox".to_string())];
    if s.repos.len() > 1 {
        for repo in &s.repos {
            dirs.push((
                std::path::Path::new(wt).join(repo).join("outbox"),
                format!("{repo}/outbox"),
            ));
        }
    }
...
pub async fn ack_session(
    ...
    let Some(event_id) = body.get("eventId").and_then(|v| v.as_i64()) else {
        return (
            StatusCode::BAD_REQUEST,
            Json(json!({"error":"missing eventId"})),
        )
            .into_response();
    };
...
mod tests {
    ...
    async fn get_unknown_session_is_404() {
        ...
        let (status, body) = oneshot_req(
            st.clone(),
            Request::get("/api/sessions/nope")
    ...
    async fn get_session_returns_session_and_filtered_log() {
        ...
        st.store
            ...
            .unwrap();
        st.store
            .append_log(id, "{\"type\":\"assistant\",\"text\":\"hi\"}")
            .await
            .unwrap();
        st.store
            .append_log(id, "{\"type\":\"tool_result\"}")
            .await
            .unwrap();
        let (status, body) = oneshot_req(
            st.clone(),
    ...
    async fn get_session_windowed_returns_tail_with_start_and_total() {
        ...
        for i in 0..5 {
            st.store
                .append_log(id, &format!("{{\"type\":\"assistant\",\"i\":{i}}}"))
                .await
                .unwrap();
        }
        let (status, body) = oneshot_req(
            st.clone(),
            Request::get(format!("/api/sessions/{id}?limit=2"))
                .header("authorization", auth(&st))
                .body(Body::empty())
                .unwrap(),
        )
    ...
    async fn create_unknown_repo_is_400() {
        ...
        let (status, _body) = oneshot_req(
            st.clone(),
            Request::post("/api/sessions")
                .header("authorization", auth(&st))
                .header("content-type", "application/json")
                .body(Body::from(r#"{"repo":"nope","prompt":"x"}"#))
                .unwrap(),
        )
        .await;
        assert_eq!(status, StatusCode::BAD_REQUEST);
    ...
    async fn message_unknown_session_is_404() {
        ...
        let (status, body) = oneshot_req(
            st.clone(),
            Request::post("/api/sessions/nope/messages")
                .header("authorization", auth(&st))
                .header("content-type", "application/json")
    ...
    async fn interrupt_with_empty_json_body_is_ok() {
        ...
        let (status, body) = oneshot_req(
            st.clone(),
            Request::post(format!("/api/sessions/{id}/interrupt"))
                .header("authorization", auth(&st))
                .header("content-type", "application/json")
                .body(Body::empty())
                .unwrap(),
    ...
    async fn workflows_no_worktree_returns_empty_not_500() {
        ...
        let (status, body) = oneshot_req(
            st.clone(),
            Request::get(format!("/api/sessions/{id}/workflows"))
                .header("authorization", auth(&st))
                .body(Body::empty())
                .unwrap(),
        )
    ...
    fn engine_error_response_maps_status_and_preserves_message() {
        ...
        let cases = [
            ...
            (
                E::BadInput("unknown repo: z".into()),
                StatusCode::BAD_REQUEST,
                "unknown repo: z",
            ),
            (
                E::SourceUnhealthy("worktree missing".into()),
                StatusCode::CONFLICT,
                "source unhealthy: worktree missing",
            ),
            (
        ...
        for (err, want_status, want_msg) in cases {
            assert_eq!(
                err.to_string(),
                want_msg,
                "Display must preserve the exact message"
            );
    ...
    async fn wait_done(st: &crate::api::state::AppState, id: &str) {
        for _ in 0..250 {
            if st.engine.get(id).await.map(|s| s.status) == Some("done".into()) {
                return;
            }
    ...
    async fn create_run_get_resume_and_delete_round_trip() {
        ...
        let (status, body) = oneshot_req(
            ...
            Request::post("/api/sessions")
                .header("authorization", auth(&st))
                .header("content-type", "application/json")
                .body(Body::from(r#"{"repo":"demo","prompt":"go"}"#))
                .unwrap(),
        )
        .await;
        assert_eq!(status, StatusCode::OK);
        ...
        let types: Vec<String> = body["log"]
            ...
            .collect();
        assert!(
            types.iter().any(|t| t == "assistant" || t == "result"),
            "filtered log must carry claude's rendered lines, got {:?}",
            body["log"]
        );
        ...
        let (status, body) = oneshot_req(
            ...
            Request::post(format!("/api/sessions/{id}/messages"))
                .header("authorization", auth(&st))
                .header("content-type", "application/json")
                .body(Body::from(r#"{"prompt":"second","setTitle":false}"#))
                .unwrap(),
        )
        .await;
        assert_eq!(status, StatusCode::OK);
        ...
        wait_done(&st, &id).await;
        let (status, _b) = oneshot_req(
            st.clone(),
            Request::post(format!("/api/sessions/{id}/delete"))
                .header("authorization", auth(&st))
                .header("content-type", "application/json")
                .body(Body::empty())
                .unwrap(),
    ...
    async fn file_route_serves_a_worktree_file_with_length_and_mime() {
        ...
        st.store
            ...
            .await
            .unwrap();
        let resp = crate::api::app(st.clone())
            .oneshot(
    ...
    async fn gzip_compresses_large_json_but_not_file_downloads() {
        ...
        for i in 0..40 {
            st.store
                .create(crate::engine::store::CreateInput {
                    ...
                    worktree_path: Some(format!("/tmp/wt/s-{i:03}")),
        ...
        st.store
            ...
            .await
            .unwrap();
        let fresp = crate::api::app(st.clone())
            .oneshot(
        ...
        assert_eq!(
            fresp.headers().get(header::CONTENT_ENCODING),
            None,
    ...
    async fn file_route_rejects_escape_and_missing() {
        ...
        let (s, b) = oneshot_req(
            st.clone(),
            Request::get(format!("/api/sessions/{id}/file"))
                .header("authorization", auth(&st))
                .body(Body::empty())
                .unwrap(),
        )
        ...
        // escape attempt → 404
        let (s2, _b) = oneshot_req(
            st.clone(),
            Request::get(format!("/api/sessions/{id}/file?path=../../etc/passwd"))
                .header("authorization", auth(&st))
                .body(Body::empty())
                .unwrap(),
        )
    ...
    async fn file_full_download_advertises_resume_headers() {
        ...
        assert!(
            etag.is_some(),
            "full download must carry an ETag for later If-Range resumes"
        );
    ...
    async fn file_etag_changes_when_file_regenerated_with_same_size_and_mtime() {
        ...
        let etag1 = file_req(&st, "s-rng-regen", &[])
            .await
            .headers()
            .get("etag")
            .and_then(|v| v.to_str().ok())
        ...
        assert_ne!(
            etag1, etag2,
            "regenerated file (same size+mtime) must not keep its ETag"
        );
    }
    ...
    async fn file_range_malformed_or_multi_ignored() {
        ...
        for h in [
            ...
            assert_eq!(
                resp.status(),
                StatusCode::OK,
                "range header {h:?} must fall back to full body"
            );
            let body = resp.into_body().collect().await.unwrap().to_bytes();
            assert_eq!(&body[..], b"0123456789");
        }
    ...
    async fn file_if_range_match_resumes_mismatch_restarts() {
        ...
        let hit = file_req(
            &st,
            "s-rng-ir",
            &[("range", "bytes=8-"), ("if-range", etag.as_str())],
        )
        .await;
    ...
    async fn outbox_lists_floored_mtimes() {
        ...
        let (s, b) = oneshot_req(
            st.clone(),
            Request::get(format!("/api/sessions/{id}/outbox"))
                .header("authorization", auth(&st))
                .body(Body::empty())
                .unwrap(),
        )
    ...
    async fn outbox_lists_nested_files_recursively() {
        ...
        assert_eq!(
            paths,
            vec![
                "outbox/logo/icon.svg",
                "outbox/logo/sub/deep.png",
                "outbox/report.md"
            ]
        );
    ...
    async fn commits_route_404_on_unknown_session() {
        ...
        let (s, b) = oneshot_req(
            st.clone(),
            Request::get("/api/sessions/nope/commits")
                .header("authorization", auth(&st))
                .body(Body::empty())
                .unwrap(),
        )
    ...
    async fn commit_diff_route_returns_hunks_for_working_change() {
        ...
        let (s, b) = oneshot_req(
            st.clone(),
            Request::get(format!(
                "/api/sessions/{id}/commits/working/diff?repo=demo&path=README.md"
            ))
            .header("authorization", auth(&st))
            .body(Body::empty())
            .unwrap(),
    ...
    async fn rewind_route_404_and_requires_turn_index() {
        ...
        let (s, _b) = oneshot_req(
            st.clone(),
            Request::post("/api/sessions/nope/rewind")
                .header("authorization", auth(&st))
                .header("content-type", "application/json")
                .body(Body::from("{\"turnIndex\":0}"))
                .unwrap(),
        )
        .await;
        assert_eq!(s, StatusCode::NOT_FOUND);
        ...
        st.store
            ...
            .await
            .unwrap();
        let (s2, b2) = oneshot_req(
            st.clone(),
            Request::post(format!("/api/sessions/{id}/rewind"))
                .header("authorization", auth(&st))
                .header("content-type", "application/json")
                .body(Body::from("{}"))
                .unwrap(),
    ...
    async fn engine_rewind_restores_snapshot_and_base() {
        ...
        std::fs::write(repo_dir.join("README.md"), "base\nturn1\n").unwrap();
        crate::engine::worktree::snapshot_worktree(
            &repo_dir,
            &format!("refs/agentic/snapshots/{id}/1"),
        )
        .unwrap();
        std::fs::write(repo_dir.join("README.md"), "base\nlater-garbage\n").unwrap();
        ...
        assert!(
            matches!(err, crate::engine::EngineError::BadInput(_)),
            "missing snapshot must be BadInput"
        );
    }
    ...
    async fn upload_between_2mb_and_max_returns_200() {
        ...
        let (status, body) = oneshot_req(
            st.clone(),
            Request::post(format!("/api/sessions/{id}/upload"))
                .header("authorization", auth(&st))
        ...
        assert_eq!(
            status,
            StatusCode::OK,
            "3 MB upload within max must succeed, got: {:?}",
            body
        );
        assert!(
            body["path"].as_str().unwrap_or("").starts_with("uploads/"),
            "expected uploads/ path"
        );
    }
    ...
    async fn upload_above_max_returns_413() {
        ...
        assert_eq!(
            status,
            StatusCode::PAYLOAD_TOO_LARGE,
            "oversize upload must return 413"
        );
    }
    ...
    async fn fork_missing_source_worktree_is_409() {
        ...
        let (status, body) = oneshot_req(
            st.clone(),
            Request::post(format!("/api/sessions/{id}/fork"))
                .header("authorization", auth(&st))
                .body(Body::empty())
                .unwrap(),
        )
        ...
        assert!(
            msg.contains("source worktree missing"),
    ...
    async fn adopt_route_creates_session_with_adopted_origin() {
        ...
        let cb =
            std::env::temp_dir().join(format!("agentic-adopt-http-{}-{n}", std::process::id()));
        ...
        let (status, body) = oneshot_req(
            st.clone(),
            Request::post("/api/sessions/adopt")
                .header("authorization", auth(&st))
                .header("content-type", "application/json")
                .body(Body::from(format!(
                    r#"{{"claudeSessionId":"{csid}","cwd":"{cwd}"}}"#
                )))
                .unwrap(),
        )
        .await;
        ...
        let (status, adoptable) = oneshot_req(
            st.clone(),
            Request::get("/api/adoptable")
                .header("authorization", auth(&st))
                .body(Body::empty())
                .unwrap(),
        )
    ...
    async fn adoptable_route_lists_seeded_transcript() {
        let n = ADOPT_CTR.fetch_add(1, AO::SeqCst);
        let cb =
            std::env::temp_dir().join(format!("agentic-adoptable-http-{}-{n}", std::process::id()));
        let _ = std::fs::remove_dir_all(&cb);
        let st = adopt_state_with_config_base(cb.clone()).await;
    ...
    async fn adopt_route_missing_transcript_is_400() {
        ...
        let (status, body) = oneshot_req(
            ...
            Request::post("/api/sessions/adopt")
                ...
                .header("content-type", "application/json")
                .body(Body::from(
                    r#"{"claudeSessionId":"nope","cwd":"/tmp/does-not-exist-xyz"}"#,
                ))
                .unwrap(),
        )
        .await;
    ...
    async fn create_session_rejects_extra_mcp_with_no_transport() {
        ...
        let (status, body) = oneshot_req(
            ...
            Request::post("/api/sessions")
                ...
                .header("content-type", "application/json")
                .body(Body::from(
                    r#"{"prompt":"test","extraMcpServers":[{"name":"bad-mcp"}]}"#,
                ))
                .unwrap(),
        )
        .await;
        ...
        let msg = body["error"].as_str().unwrap_or("");
        assert!(
            msg.contains("must have either command"),
    ...
    async fn create_session_rejects_extra_mcp_with_empty_name() {
        ...
        let (status, body) = oneshot_req(
            ...
            Request::post("/api/sessions")
                ...
                .header("content-type", "application/json")
                .body(Body::from(
                    r#"{"prompt":"test","extraMcpServers":[{"name":"","command":"npx"}]}"#,
                ))
                .unwrap(),
        )
        .await;
        ...
        let msg = body["error"].as_str().unwrap_or("");
        assert!(
            msg.contains("name must be non-empty"),
    ...
    async fn detach_route_unknown_session_is_400() {
        ...
        let (status, body) = oneshot_req(
            st.clone(),
            Request::post("/api/sessions/nope/detach")
                .header("authorization", auth(&st))
                .body(Body::empty())
                .unwrap(),
        )
    ...
    async fn create_session_rejects_reserved_agentic_name() {
        ...
        let (status, body) = oneshot_req(
            ...
            Request::post("/api/sessions")
                ...
                .header("content-type", "application/json")
                .body(Body::from(
                    r#"{"prompt":"test","extraMcpServers":[{"name":"agentic","command":"npx"}]}"#,
                ))
                .unwrap(),
        )
        .await;
    ...
    async fn create_session_rejects_empty_command() {
        ...
        let (status, body) = oneshot_req(
            ...
            Request::post("/api/sessions")
                ...
                .header("content-type", "application/json")
                .body(Body::from(
                    r#"{"prompt":"test","extraMcpServers":[{"name":"my-mcp","command":""}]}"#,
                ))
                .unwrap(),
        )
        .await;
    ...
    async fn create_session_rejects_whitespace_command() {
        ...
        assert!(
            msg.contains("command must be non-empty"),
            "expected whitespace-command error, got: {msg}"
        );
    }
    ...
    async fn create_session_rejects_empty_url() {
        ...
        let (status, body) = oneshot_req(
            ...
            Request::post("/api/sessions")
                ...
                .header("content-type", "application/json")
                .body(Body::from(
                    r#"{"prompt":"test","extraMcpServers":[{"name":"my-mcp","url":""}]}"#,
                ))
                .unwrap(),
        )
        .await;
        ...
        let msg = body["error"].as_str().unwrap_or("");
        assert!(
            msg.contains("url must be non-empty"),
            "expected empty-url error, got: {msg}"
        );
    }
    ...
    fn validate_disjoint_catches_conflict() {
        ...
        assert!(
            msg.contains("gh@m"),
            "error must name the conflicting id: {msg}"
        );
    }

    #[test]
    fn validate_disjoint_allows_non_overlapping_lists() {
        let err = validate_disjoint(&["gh@m".to_string()], &["sp@m".to_string()], "plugins");
        assert!(err.is_none());
    }
    ...
    fn validate_disjoint_catches_whitespace_padded_conflict() {
        ...
        assert!(
            validate_disjoint(&["gh@m".to_string()], &["gh@m ".to_string()], "plugins").is_some()
        );
        assert!(validate_disjoint(
            &[" rke2-ops".to_string()],
            &["rke2-ops".to_string()],
            "skills"
        )
        ...
        assert!(
            validate_disjoint(&["gh@m".to_string()], &["GH@m".to_string()], "plugins").is_none()
        );
    }
    ...
    async fn create_session_rejects_same_id_in_hidden_and_forced_on() {
        ...
        let (status, body) = oneshot_req(
            ...
            Request::post("/api/sessions")
                ...
                .header("content-type", "application/json")
                .body(Body::from(
                    r#"{"prompt":"test","hiddenPlugins":["gh@m"],"forcedOnPlugins":["gh@m"]}"#,
                ))
                .unwrap(),
        )
        .await;
        ...
        assert!(
            msg.contains("gh@m"),
            "error must name conflicting id: {msg}"
        );
        // Skill conflict
        ...
        assert!(
            msg2.contains("rke2-ops"),
            "error must name conflicting id: {msg2}"
        );
    }
}
