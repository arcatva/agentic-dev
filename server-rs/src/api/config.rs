...
impl Config {
    pub fn load(get: impl Fn(&str) -> Option<String>) -> Config {
        ...
        let src = get("AGENTIC_SRC_ROOT")
            .unwrap_or_else(|| join(&home, "src").to_string_lossy().into_owned());
        let data_dir = get("AGENTIC_DATA_DIR")
            .unwrap_or_else(|| join(&home, ".agentic-dev").to_string_lossy().into_owned());
        Config {
            src_root: PathBuf::from(&src),
            worktrees_root: get("AGENTIC_WORKTREES_ROOT")
                .map(PathBuf::from)
                .unwrap_or_else(|| join(&src, "agentic-worktrees")),
            ...
            claude_config_base: get("AGENTIC_CLAUDE_CONFIG_BASE")
                .map(PathBuf::from)
                .unwrap_or_else(|| join(&home, ".claude")),
            port: get("AGENTIC_PORT")
            ...
            tls_cert: get("AGENTIC_TLS_CERT")
                .filter(|s| !s.is_empty())
                .map(PathBuf::from),
            tls_key: get("AGENTIC_TLS_KEY")
                .filter(|s| !s.is_empty())
                .map(PathBuf::from),
            tls_dir: get("AGENTIC_TLS_DIR")
                .filter(|s| !s.is_empty())
                .map(PathBuf::from)
                .unwrap_or_else(|| PathBuf::from(&data_dir).join("tls")),
            tls_extra_sans: get("AGENTIC_TLS_SAN")
                .map(|s| split_csv(&s))
                .unwrap_or_default(),
            tls_regen: get("AGENTIC_TLS_REGEN")
                .map(|s| is_truthy(&s))
                .unwrap_or(false),
            transcript_cache_bytes: get("AGENTIC_TRANSCRIPT_CACHE_BYTES")
                .and_then(|v| v.parse().ok())
                .unwrap_or(256 * 1024 * 1024),
            // Phase 6 additions
            skills_dir: get("AGENTIC_SKILLS_DIR")
                .map(PathBuf::from)
                .unwrap_or_else(|| PathBuf::from(&home).join(".claude").join("skills")),
            groups_path: get("AGENTIC_GROUPS_PATH")
                .map(PathBuf::from)
                .unwrap_or_else(|| PathBuf::from(&data_dir).join("groups.json")),
            templates_path: get("AGENTIC_TEMPLATES_PATH")
                .map(PathBuf::from)
                .unwrap_or_else(|| PathBuf::from(&data_dir).join("templates.json")),
            device_token_path: get("AGENTIC_DEVICE_TOKEN_PATH")
                .map(PathBuf::from)
                .unwrap_or_else(|| PathBuf::from(&data_dir).join("device.json")),
            upload_max_bytes: get("AGENTIC_UPLOAD_MAX_BYTES")
                .and_then(|v| v.parse().ok())
                .unwrap_or(64 * 1024 * 1024),
            // Watchdog timers: seconds → ms. Unset/non-numeric → None.
            idle_max_ms: get("AGENTIC_TURN_IDLE_SEC")
                .and_then(|v| v.parse::<i64>().ok())
                .map(|s| s * 1000),
            wall_max_ms: get("AGENTIC_TURN_WALL_SEC")
                .and_then(|v| v.parse::<i64>().ok())
                .map(|s| s * 1000),
            idle_ttl_ms: get("AGENTIC_IDLE_TTL_SEC")
                .and_then(|v| v.parse::<i64>().ok())
                .map(|s| s * 1000),
...
mod tests {
    ...
    fn defaults_are_correct() {
        ...
        assert_eq!(
            c.db_path.to_str().unwrap(),
            "/home/u/.agentic-dev/db.sqlite"
        );
    ...
    fn transcript_cache_bytes_default_and_override() {
        ...
        let c2 = Config::load(env_of(&[
            ("HOME", "/home/u"),
            ("AGENTIC_TRANSCRIPT_CACHE_BYTES", "1048576"),
        ]));
    ...
    fn phase6_path_defaults_and_overrides() {
        ...
        assert_eq!(
            c.groups_path.to_str().unwrap(),
            "/home/u/.agentic-dev/groups.json"
        );
        assert_eq!(
            c.templates_path.to_str().unwrap(),
            "/home/u/.agentic-dev/templates.json"
        );
        assert_eq!(
            c.device_token_path.to_str().unwrap(),
            "/home/u/.agentic-dev/device.json"
        );
        assert_eq!(c.upload_max_bytes, 64 * 1024 * 1024);
        let c2 = Config::load(env_of(&[
            ("HOME", "/home/u"),
            ("AGENTIC_UPLOAD_MAX_BYTES", "10"),
        ]));
    ...
    fn tls_byo_and_sans_env_parse() {
        ...
        assert_eq!(
            c.tls_cert.as_ref().unwrap().to_str().unwrap(),
            "/etc/ssl/fullchain.pem"
        );
        assert_eq!(
            c.tls_key.as_ref().unwrap().to_str().unwrap(),
            "/etc/ssl/privkey.pem"
        );
        assert_eq!(c.tls_dir.to_str().unwrap(), "/var/agtls");
        assert_eq!(
            c.tls_extra_sans,
            vec!["10.0.0.5".to_string(), "agentic.lan".to_string()]
        );
        assert!(c.tls_regen);
