...
pub struct WorkflowAgent {
    ...
    pub state: String,
    pub model: String,
    #[serde(rename = "phaseTitle", skip_serializing_if = "Option::is_none")]
    pub phase_title: Option<String>,
    #[serde(rename = "promptPreview", skip_serializing_if = "Option::is_none")]
...
fn project_dirs(base: &Path, session_uuid: Option<&str>) -> Vec<PathBuf> {
    ...
    for slug in slugs.flatten() {
        ...
        let Ok(sids) = std::fs::read_dir(&sp) else {
            continue;
        };
...
fn birthtime_or_mtime_ms(p: &Path) -> Option<i64> {
    let md = std::fs::metadata(p).ok()?;
    if let Ok(d) = md.created().and_then(|t| {
    ...
    if let Ok(entries) = std::fs::read_dir(dir.join("workflows").join("scripts")) {
        for e in entries.flatten() {
            ...
            if n.ends_with(&suffix) {
                return n[..n.len() - suffix.len()].to_string();
            }
        }
    }
    "workflow".to_string()
    ...
    if let Ok(entries) = std::fs::read_dir(dir.join("workflows").join("scripts")) {
        for e in entries.flatten() {
            if e.file_name().to_string_lossy().ends_with(&suffix) {
                if let Some(ms) = mtime_ms(&e.path()) {
                    return ms;
                } // scripts file → mtime
            }
        }
...
pub(crate) fn read_running_run(dir: &Path, run_dir: &Path, run_id: &str) -> Option<WorkflowRun> {
    ...
    let mut metas: std::collections::HashMap<String, serde_json::Value> =
    ...
    let agents = ids
        ...
        .map(|(i, id)| {
            ...
            WorkflowAgent {
                ...
                phase_title: m
                    .and_then(|m| m.get("phaseTitle").and_then(|v| v.as_str()))
                    .map(String::from),
                prompt_preview: m
                    .and_then(|m| m.get("promptPreview").and_then(|v| v.as_str()))
                    .map(String::from),
    ...
    let run_meta = std::fs::read_to_string(run_dir.join("run.meta.json"))
        .ok()
        .and_then(|t| serde_json::from_str::<serde_json::Value>(&t).ok());
    let name = run_meta
        .as_ref()
    ...
    Some(WorkflowRun {
        ...
        status: "running".into(),
        summary: None,
        agent_count: Some(ids.len() as i64),
            ...
            for e in entries.flatten() {
                ...
                let agents = d
                    ...
                    .map(|arr| {
                        arr.iter()
                            .filter(|a| {
                                a.get("type").and_then(|t| t.as_str()) == Some("workflow_agent")
                            })
                            .map(|a| WorkflowAgent {
                                agent_id: a
                                    .get("agentId")
...
pub fn read_workflow_agent(
    ...
    if !safe_segment(run_id) || !safe_segment(agent_id) {
        return String::new();
    }
...
mod tests {
    ...
    fn running_run_backfills_native_worker_model_from_transcript() {
        ...
        std::fs::write(
            run.join("agent-w1.meta.json"),
            serde_json::json!({"label":"w","model":""}).to_string(),
        )
        .unwrap();
        std::fs::write(
        ...
        std::fs::write(
            run.join("agent-w2.meta.json"),
            serde_json::json!({"label":"w","model":"MiniMax-M3"}).to_string(),
        )
        .unwrap();
        std::fs::write(
            run.join("agent-w2.jsonl"),
            "{\"type\":\"system\",\"subtype\":\"init\",\"model\":\"claude-opus-4-8[1m]\"}\n",
        )
        .unwrap();
    ...
    fn summary_wins_over_live_journal_for_same_runid() {
        ...
        std::fs::write(
            sd.join("workflows").join("dup.json"),
            serde_json::json!({"runId":"dup","status":"done"}).to_string(),
        )
        .unwrap();
    ...
    fn summary_runid_falls_back_to_filename_and_skips_malformed_sibling() {
        ...
        std::fs::write(
            wf.join("noid.json"),
            serde_json::json!({"workflowName":"derived","status":"failed"}).to_string(),
        )
        .unwrap();
    ...
    fn zero_created_at_falls_back_to_file_time_not_zero() {
        ...
        assert!(
            runs[0].created_at > 0,
            "createdAt should fall back to file time, got {}",
            runs[0].created_at
        );
        std::fs::remove_dir_all(&b).ok();
    }
