...
impl Engine {
    ...
    pub async fn commit_graph(&self, id: &str) -> Result<serde_json::Value, EngineError> {
        ...
        for h in handles {
            out.push(
                h.await
                    .map_err(|e| EngineError::Internal(format!("commit_graph task: {e}")))?,
            );
        }
    ...
    pub async fn commit_files(
        ...
        repo: &str,
        sha: &str,
    ) -> Result<Vec<crate::engine::structured_diff::CommitFile>, EngineError> {
        let s = self
            .get(id)
            .await
            .ok_or_else(|| EngineError::NotFound(id.to_string()))?;
        if !s.repos.iter().any(|r| r == repo) {
            return Err(EngineError::BadInput(format!("unknown repo: {repo}")));
        }
        let valid_sha = sha == "working"
            || (sha.len() >= 4 && sha.len() <= 40 && sha.chars().all(|c| c.is_ascii_hexdigit()));
        if !valid_sha {
            return Err(EngineError::BadInput(format!("bad sha: {sha}")));
        }
        let wt_root = s.worktree_path.clone().ok_or(EngineError::NoWorktree)?;
        let wt = std::path::Path::new(&wt_root).join(repo);
    ...
    pub async fn commit_diff(
        ...
        if path.is_empty() {
            return Err(EngineError::BadInput("path required".to_string()));
        }
