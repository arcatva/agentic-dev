...
pub fn fcm_body(payload: &PushPayload, device_token: &str) -> serde_json::Value {
    ...
    let body = if payload.is_error {
        format!(
            "Error: {}",
            payload.error_text.as_deref().unwrap_or("unknown error")
        )
    } else {
    ...
    let cost_str = match payload.cost_usd {
        Some(c) => c.to_string(),
        None => String::new(),
    };
    serde_json::json!({
...
pub async fn send_push(
    ...
    let (Some(token), Some(creds)) = (device_token.filter(|t| !t.is_empty()), creds) else {
        return;
    };
...
mod tests {
    ...
    fn creds_require_both_env_vars() {
        ...
        assert!(load_fcm_creds(|k| if k == "AGENTIC_FCM_PROJECT_ID" {
            Some("p".into())
        } else {
            None
        })
    ...
    async fn no_op_when_token_or_creds_missing() {
        ...
        let c = calls.clone();
        let f = move |_b: serde_json::Value| {
            *c.lock().unwrap() += 1;
        };
        let creds = FcmCreds {
            project_id: "p".into(),
    ...
    fn title_prefers_session_title_first_line_capped() {
        ...
        let p = PushPayload {
            title: Some("修复Session选择器UI布局\n第二行不应出现".into()),
            ..payload()
        };
        let v = fcm_body(&p, "tok");
        assert_eq!(
            v["message"]["notification"]["title"],
            "修复Session选择器UI布局"
        );
        ...
        let t = fcm_body(&p2, "tok")["message"]["notification"]["title"]
            .as_str()
            .unwrap()
            .to_string();
        ...
        assert_eq!(
            fcm_body(&p4, "tok")["message"]["notification"]["title"],
            "real"
        );
    }
