...
mod submit_titles_via_generator {
    ...
    async fn with_activity_does_not_reemit_outbox_markers_after_state_loss() {
        ...
        assert_eq!(file_marker_count(&e, "s-outbox"), 1, "first touch marks the file once");
        let _ = e.with_activity(s.clone());
        assert_eq!(file_marker_count(&e, "s-outbox"), 1, "same-process re-touch never re-marks");
        ...
        let _ = e.with_activity(s.clone());
        assert_eq!(file_marker_count(&e, "s-outbox"), 2, "new file after restart marks once");
        let _ = e.with_activity(s.clone());
    ...
    async fn with_activity_seeding_covers_the_single_repo_outbox_layout() {
        ...
        let _ = e.with_activity(s.clone());
        assert_eq!(file_marker_count(&e, "s-outbox-1repo"), 1, "first touch marks once");
