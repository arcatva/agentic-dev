...
mod submit_titles_via_generator {
    ...
    async fn concurrent_double_adopt_same_csid_exactly_one_succeeds() {
        ...
        let cwd_s = cwd.to_string_lossy().to_string();
        let tp =
            native_transcript::transcript_path(&e.0.cfg.claude_config_base, &cwd_s, "csidRACE");
        std::fs::create_dir_all(tp.parent().unwrap()).unwrap();
        std::fs::write(
            &tp,
            "{\"type\":\"user\",\"message\":{\"role\":\"user\",\"content\":\"hi\"}}\n",
    ...
    async fn followup_reclaim_leaves_detached_true_when_transcript_missing() {
        ...
        let cwd_s = cwd.to_string_lossy().to_string();
        let tp =
            native_transcript::transcript_path(&e.0.cfg.claude_config_base, &cwd_s, "csidMISS");
        std::fs::create_dir_all(tp.parent().unwrap()).unwrap();
        std::fs::write(
            &tp,
            "{\"type\":\"user\",\"message\":{\"role\":\"user\",\"content\":\"hi\"}}\n",
