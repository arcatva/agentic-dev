...
pub fn list_components(config_base: &Path, skills_dir: &Path) -> Vec<ComponentInfo> {
    ...
    for s in crate::engine::skills::list_skills(skills_dir) {
        let global_enabled =
            crate::engine::global_settings::skill_globally_enabled(&toggles, &s.name);
        out.push(ComponentInfo {
            kind: "skill".into(),
            id: s.name.clone(),
            name: s.name,
    ...
    for p in crate::engine::plugins::list_plugins(config_base) {
        ...
        let global_enabled =
            crate::engine::global_settings::plugin_globally_enabled(&toggles, &p.name);
        out.push(ComponentInfo {
            kind: "plugin".into(),
            id: p.name.clone(),
            name: display,
...
mod tests {
    ...
    fn list_components_includes_mcp_servers() {
        ...
        let mcp = out
            .iter()
            .find(|c| c.kind == "mcp" && c.id == "test-mcp")
            .unwrap();
    ...
    fn merges_skills_and_plugins_with_global_enabled() {
        ...
        std::fs::write(
            ...
            r#"{"version":2,"plugins":{"github@mkt":[{"scope":"user"}]}}"#,
        )
        .unwrap();
        ...
        let skill = out
            .iter()
            .find(|c| c.kind == "skill" && c.id == "rke2-ops")
            .unwrap();
        ...
        let plugin = out
            .iter()
            .find(|c| c.kind == "plugin" && c.id == "github@mkt")
            .unwrap();
